...
- `availability` y `source` son dos ejes independientes, no un único
  valor: `availability` (`free | premium | coming-soon`) es el estado
  comercial — ¿cuesta dinero? — y `source` (`internal | external`) es
  la procedencia — ¿el enlace es propio del Hub o de un tercero? Un
  recurso puede ser, por ejemplo, `premium` + `external` (de pago,
  alojado en una tienda de terceros) o `free` + `internal` (gratis,
  servido por el propio Hub): las seis combinaciones son válidas por
  diseño.
- `url` es **obligatoria** salvo cuando `availability` es
  `"coming-soon"` (ahí puede faltar). Un recurso `free`/`premium` sin
  `url` no pasa la validación del Content Layer, sea cual sea su
  `source`.
- Cuando `url` está presente, su forma válida depende de `source`:
  - `source: "external"` — debe ser una URL real `http://`/`https://`
    (el destino es forzosamente una plataforma de terceros).
  - `source: "internal"` — puede ser una URL real `http://`/`https://`
    (una vez exista dominio propio), **o** una ruta local absoluta bajo
    `/downloads/...` (carpeta `public/downloads/`) para archivos
    servidos directamente por el propio Hub (p. ej. un preset o sample
    distribuido como descarga propia, sin depender de un dominio).
  - En ambos casos se rechazan protocolos no válidos para un destino
    web público (`javascript:`, `data:`, `ftp:`, etc.).
...
Los valores internos del schema (`platform`, `type`, `availability`,
`source`...) no se muestran nunca tal cual en la interfaz:
`ContentCard`/`ResourceCard` usan mapas tipados de etiquetas
(`platformLabel`, `typeLabel`, `availabilityLabel`) para traducir a español
lo que se ve en pantalla, sin tocar los valores del schema; `ResourceCard`
muestra además un badge adicional ("Enlace externo") solo cuando
`source === 'external'`. Los mismos valores internos sí se siguen usando,
sin traducir, como
